...
function SnapApi:SnapVector(Plot, model, vector, rotation)
    ...
    if (RunService:IsServer()) then
        ...
        return Plot.Main.CFrame:ToObjectSpace(worldSpace)
    end
...
function SnapApi:GetPlotData(plotObject)
    local plotSize = plotObject.VisualPart.Size

    local back = Vector3.new(0, -1, 0)
    local top = Vector3.new(0, 0, -1)
    local right = Vector3.new(-1, 0, 0)

    local plotCFrame = plotObject.VisualPart.CFrame * CFrame.fromMatrix(-back * plotSize / 2, right, top, back)
